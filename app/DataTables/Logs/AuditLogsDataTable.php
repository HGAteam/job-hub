...
class AuditLogsDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('properties', function (Activity $model) {
                ...
                return view('pages.dashboard.log.audit._details', compact('content'));
            })
            ->editColumn('created_at', function (Activity $model) {
                return $model->created_at->format('d M, Y H:i:s');
            })
            ->addColumn('action', function (Activity $model) {
                return view('pages.dashboard.log.audit._action-menu', compact('model'));
            });
    }
    ...
    {
        return [
            Column::make('id')->title(trans('Log ID'))->addClass('ps-0'),
            Column::make('log_name')->title(trans('Location')),
            Column::make('description')->title(trans('Description')),
            Column::make('subject_type')->title(trans('Subject Type')),
            Column::make('subject_id')->title(trans('Subject')),
            Column::make('causer_id')->title(trans('Causer')),
