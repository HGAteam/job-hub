...
class Theme {
    ...
    */
    public static function init($theme, $demo = '') {
        ...
        // For multi demo, merge dashboard menu config with current demo
        if (self::$demo != '' && self::$demo != 'dashboard') {
            $dashboard_actual_path = str_replace(self::$demo, 'dashboard/', $_THEME_PATH);
            $dashboard_general_config = include $dashboard_actual_path . '/dist/config/general.php';
            $dashboard_menu_config = include $dashboard_actual_path . '/dist/config/menu.php';
            $dashboard_pages_config = include $dashboard_actual_path . '/dist/config/pages.php';

            $common_config['product'] = array_replace_recursive($common_config['product'], $dashboard_general_config['product']);
            $common_config['meta'] = array_replace_recursive($common_config['meta'], $dashboard_general_config['meta']);
            $theme_config['menu'] = array_replace_recursive($dashboard_menu_config, $theme_config['menu']);
            $theme_config['pages'] = array_replace_recursive($dashboard_pages_config, $theme_config['pages']);
        }
    ...
    */
    public static function setViewMode($value) {
        ...
        if (isset($_SERVER['SERVER_NAME']) && strpos($_SERVER['SERVER_NAME'], 'hgateam.com') !== false) {
    ...
    public static function getView($path, $params = array(), $once = false, $createPartialFileInExport = true) {
        ...
        // For multi demo, include from dashboard for other demos
        if ( file_exists($actual_path) === false ) {
            if (self::$demo != '' && self::$demo != 'dashboard') {
                $actual_path = str_replace(self::$demo . '/', 'dashboard/', $actual_path);
            }
        }
    ...
    public static function getChangelogInfo() {
        ...
        if ( file_exists($current_path) === false ) {
            if (self::$demo != '' && self::$demo != 'dashboard') {
                $current_path = str_replace(self::$demo . '/', 'dashboard/', $current_path);
            }
        }
