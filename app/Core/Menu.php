    ...
    */
    ...
    private function _generateItemLinkTitle($item) {
        ...
        if ( isset($item['classes']) && isset($item['classes']['title']) ) {
            $classes[] = trans($item['classes']['title']);
        }

        if (!is_string($item['title']) && is_callable($item['title'])) {
            $item['title'] = trans(call_user_func($item['title'], $item));
        }

        if ( isset($this->callbacks['title']) && is_callable($this->callbacks['title']) ) {
            $title = trans(call_user_func($this->callbacks['title'], $item, $item['title']));
        } else {
    ...
    private function _generateItemHeading($item) {
        ...
        if ( isset($item['heading']) ) {
            ...
            if ( isset($this->callbacks['heading']) && is_callable($this->callbacks['heading']) ) {
                echo call_user_func($this->callbacks['heading'], trans($item['heading']));
            } else {
                echo '<h3 ' . Util::getHtmlClass($classes) . '>';
                echo trans($item['heading']);
                echo '</h3>';
            }
    ...
    private function _generateItemContent($item) {
        ...
        if ( isset($item['content']) ) {
            ...
            // echo $item['content'];
            // Aplicar la traducción solo al contenido dentro de las etiquetas <span></span>
            $translatedContent = preg_replace_callback(
                '/(<span[^>]*>)(.*?)(<\/span>)/',
                function ($matches) {
                    return $matches[1] . trans($matches[2]) . $matches[3];
                },
                $item['content']
            );
    ...
    private function _matchItemByPath($item) {
        if ( isset($item['path']) && ($this->path === $item['path'] || $this->path === $item['path'] . '/home') ) {
            return true;
        } else {
            return false;
        }
    ...
    private function _buildBreadcrumb($items, &$breadcrumb, $options, $level = 0) {
        ...
        foreach ( $items as $item ) {
            ...
            if (isset($item['breadcrumb-title'])) {
                $title = trans($item['breadcrumb-title']);
            } else if (isset($item['title'])) {
                if (!is_string($item['title']) && is_callable($item['title'])) {
                    $title = call_user_func(trans($item['title']), $item);
                } else {
                    $title = trans($item['title']);
                }
            } else if (isset($item['heading'])) {
                $title = trans($item['heading']);
            }

            if ( isset($item['path']) && ($item['path'] === $this->path || $item['path'] . '/home' === $this->path)) {
